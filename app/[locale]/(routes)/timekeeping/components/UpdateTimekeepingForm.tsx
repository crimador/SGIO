...
export function UpdateTimekeepingForm({ initialData, setOpen }: UpdateTimekeepingFormProps) {
  ...
  const onSubmit = async (data: UpdateTimekeepingFormValues) => {
    ...
    try {
      ...
      toast({
        title: 'Success',
        description: 'Timekeeping updated successfully',
      });
      setOpen(false);
    } catch (error: any) {
      toast({
                        ...
                        <PopoverTrigger asChild>
                          <FormControl>
                            <button
                              type="button"
                              className={cn(
                                'flex w-[240px] h-9 items-center rounded-md border px-3 text-left text-sm font-normal transition-colors hover:bg-gray-50',
                          ...
                          <FormControl>
                            ...
                            >
                              {field.value ? (
                                ...
                                <span>Pick date and time (optional)</span>
                              )}
                              <CalendarIcon className="ml-auto h-4 w-4 opacity-50" />
                            </button>
                          </FormControl>
