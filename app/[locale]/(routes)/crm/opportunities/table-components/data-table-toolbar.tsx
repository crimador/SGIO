  ...
  return (
    <div className="flex items-center justify-between">
      <div className="flex flex-1 flex-wrap items-center gap-2">
        ...
        <DateRangeFilter column={table.getColumn('close_date')} title="Clôture" />
        <NumberRangeFilter column={table.getColumn('budget')} title="Budget" unit="FCFA" />
        {isFiltered && (
          <button
      ...
      <div className="flex items-center gap-2">
        <ExportCsvButton
          table={table}
          filename={`opportunites_${format(new Date(), 'yyyy-MM-dd')}`}
          getExportRow={(row: any) => ({
            'Nom': row.name ?? '',
            'Stade de vente': row.sales_stage ?? '',
            'Budget (FCFA)': row.budget ?? 0,
            'Revenu prévu (FCFA)': row.expected_revenue ?? 0,
            'Devise': row.currency ?? 'XOF',
            'Date clôture': row.close_date ? format(new Date(row.close_date), 'dd/MM/yyyy') : '',
            'Statut': row.status ?? '',
