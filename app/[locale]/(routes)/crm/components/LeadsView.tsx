...
const LeadsView = ({ data, crmData, canWrite = true }: any) => {
  ...
  return (
    <Card className="overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-0.5 text-xs text-gray-400">
              {data?.length ?? 0} prospect{(data?.length ?? 0) > 1 ? 's' : ''}
            </p>
          </div>
          {canWrite && (
            <Sheet open={open} onOpenChange={setOpen}>
              ...
              <SheetContent className="min-w-[800px] space-y-2">
                <SheetHeader><SheetTitle>Nouveau prospect</SheetTitle></SheetHeader>
      ...
      <CardContent className="pt-0">
        {!data || data.length === 0 ? (
          <p className="py-6 text-center text-sm text-gray-400">Aucun prospect trouvé.</p>
        ) : (
